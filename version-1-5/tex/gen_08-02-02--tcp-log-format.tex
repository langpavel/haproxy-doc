...
The TCP format is used when \emph{option tcplog} is specified in the frontend, and
...
emitted earlier if \emph{option logasap} is specified, which makes sense in most
environments with long sessions such as remote terminals. Sessions which match
the \emph{monitor} rules are never logged. It is also possible not to emit logs for
sessions for which no data were exchanged between the client and the server, by
specifying \emph{option dontlognull} in the frontend. Successful connections will
...
marked with a star \CHAR{*} after the field name below.
